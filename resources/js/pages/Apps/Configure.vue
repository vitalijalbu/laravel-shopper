      ...
      <div class="header-content">
        <div class="app-info">
          ...
          <div class="app-details">
            <h1 class="app-name">
              {{ t("apps.configure.title", { name: app.name }) }}
            </h1>
            <p class="app-version">
              {{ t("apps.details.version") }} {{ app.version }}
            </p>
        ...
        <div class="header-actions">
          <Link :href="route('apps.installed')" class="btn btn-outline">
          ...
          >
            ...
            {{
              testing
                ? t("apps.configure.testing")
                : t("apps.configure.test_connection")
            }}
          </button>
        </div>
      </div>
            ...
            <div class="settings-grid">
              <div class="setting-group">
                ...
                <p class="setting-help">
                  {{ t("apps.configure.app_name_help") }}
                </p>
              </div>

              <div class="setting-group">
                <label class="setting-label">{{
                  t("apps.configure.enabled")
                }}</label>
                ...
                <p class="setting-help">
                  {{ t("apps.configure.enabled_help") }}
                </p>
              </div>

              <div class="setting-group">
                <label class="setting-label">{{
                  t("apps.configure.auto_update")
                }}</label>
                <div class="toggle-switch">
                  <input
                ...
                <p class="setting-help">
                  {{ t("apps.configure.auto_update_help") }}
                </p>
              </div>

              <div class="setting-group">
                <label class="setting-label">{{
                  t("apps.configure.debug_mode")
                }}</label>
                <div class="toggle-switch">
                  <input
                ...
                <p class="setting-help">
                  {{ t("apps.configure.debug_mode_help") }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div v-if="activeTab === 'api'" class="tab-pane">
          <div class="settings-section">
            <h3 class="section-title">
              {{ t("apps.configure.api_settings") }}
            </h3>
            <div class="settings-grid">
              <div class="setting-group">
                <label class="setting-label">{{
                  t("apps.configure.api_url")
                }}</label>
                <input
                ...
                />
                <p class="setting-help">
                  {{ t("apps.configure.api_url_help") }}
                </p>
              </div>

              <div class="setting-group">
                <label class="setting-label">{{
                  t("apps.configure.api_key")
                }}</label>
                ...
                <p class="setting-help">
                  {{ t("apps.configure.api_key_help") }}
                </p>
              </div>

              <div class="setting-group">
                <label class="setting-label">{{
                  t("apps.configure.timeout")
                }}</label>
                <div class="input-with-suffix">
                  ...
                  <span class="input-suffix">{{
                    t("apps.configure.seconds")
                  }}</span>
                </div>
                <p class="setting-help">
                  {{ t("apps.configure.timeout_help") }}
                </p>
              </div>

              <div class="setting-group">
                <label class="setting-label">{{
                  t("apps.configure.rate_limit")
                }}</label>
                <div class="input-with-suffix">
                  <input
                  ...
                  />
                  <span class="input-suffix">{{
                    t("apps.configure.per_minute")
                  }}</span>
                </div>
                <p class="setting-help">
                  {{ t("apps.configure.rate_limit_help") }}
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="settings-section">
              ...
              >
                <div class="webhook-content">
                  <div class="webhook-header">
                    <h4 class="webhook-name">
                      {{ webhook.name || t("apps.webhooks.unnamed") }}
                    </h4>
                    <div
                      class="webhook-status"
                      :class="webhook.enabled ? 'enabled' : 'disabled'"
                    >
                      {{
                        webhook.enabled
                          ? t("apps.webhooks.enabled")
                          : t("apps.webhooks.disabled")
                      }}
                    </div>
                  </div>
                  ...
                  <div class="webhook-details">
                    <div class="detail-item">
                      <span class="detail-label"
                        >{{ t("apps.webhooks.endpoint") }}:</span
                      >
                      <span class="detail-value">{{ webhook.endpoint }}</span>
                    </div>
                    <div class="detail-item">
                      <span class="detail-label"
                        >{{ t("apps.webhooks.events") }}:</span
                      >
                      <span class="detail-value">{{
                        webhook.events?.join(", ") || "None"
                      }}</span>
                    </div>
                    <div v-if="webhook.last_success" class="detail-item">
                      <span class="detail-label"
                        >{{ t("apps.webhooks.last_success") }}:</span
                      >
                      <span class="detail-value">{{
                        formatDateTime(webhook.last_success)
                      }}</span>
                    </div>
                  </div>
                </div>
            ...
            <div v-else class="empty-state">
              ...
              <p class="empty-description">
                {{ t("apps.webhooks.add_first_webhook") }}
              </p>
            </div>
          </div>
        </div>
                ...
                <div class="permission-content">
                  <div class="permission-header">
                    <h4 class="permission-name">
                      {{ t(`apps.permissions.${permission.key}`) }}
                    </h4>
                    <div class="toggle-switch">
                      <input
          ...
          <div class="settings-section">
            ...
            <div class="advanced-actions">
              <div class="action-card">
                <div class="action-content">
                  ...
                  <p class="action-description">
                    {{ t("apps.configure.export_settings_description") }}
                  </p>
                </div>
              ...
              <div class="action-card">
                <div class="action-content">
                  <h4 class="action-title">
                    {{ t("apps.actions.import_settings") }}
                  </h4>
                  <p class="action-description">
                    {{ t("apps.configure.import_settings_description") }}
                  </p>
                </div>
                <input
                  ref="importInput"
              ...
              <div class="action-card danger">
                <div class="action-content">
                  ...
                  <p class="action-description">
                    {{ t("apps.configure.reset_settings_description") }}
                  </p>
                </div>
    ...
    <div class="configure-footer">
      <div class="footer-actions">
        ...
        >
          {{
            saving
              ? t("apps.actions.saving")
              : t("apps.configure.save_settings")
          }}
        </button>
      </div>
    </div>
...
const tabs = computed(() => [
  ...
  {
    key: "permissions",
    label: t("apps.configure.permissions"),
    icon: ShieldCheckIcon,
  },
  {
    key: "advanced",
    label: t("apps.configure.advanced"),
...
const testConnection = async () => {
  ...
  try {
    await router.post(
      route("apps.test-connection", props.app.id),
      {
...
const saveSettings = async () => {
  ...
  try {
    await router.put(
      route("apps.configure", props.app.id),
      {
        settings: settings.value,
...
const testWebhook = async (webhook) => {
  try {
    await router.post(
      route("apps.test-webhook", props.app.id),
      {
        webhook: webhook,
      },
      {
