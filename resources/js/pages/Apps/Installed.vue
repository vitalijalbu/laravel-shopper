  ...
  <div class="installed-apps">
    ...
    <div class="apps-header mb-8">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          <Link :href="route('apps.store')" class="btn btn-primary">
            {{ t("apps.store.browse") }}
          </Link>
        </div>
      </div>
    ...
    <div v-else class="empty-state">
      ...
      <h3 class="empty-title">
        {{
          activeFilter === "all"
            ? t("apps.installed.no_apps")
            : t(`apps.installed.no_${activeFilter}_apps`)
        }}
      </h3>
      <p class="empty-description">
        {{
          activeFilter === "all"
            ? t("apps.installed.install_first_app")
            : t(`apps.installed.no_${activeFilter}_description`)
        }}
      </p>
      <Link
...
const inactiveApps = computed(() =>
  props.installedApps.filter(
    (installation) => installation.status === "inactive",
  ),
);
...
const handleActivate = async (installation) => {
  try {
    loading.value = true;
    await router.post(
      route("apps.activate", installation.id),
      {},
      {
        preserveScroll: true,
        onSuccess: () => {
...
const handleDeactivate = async (installation) => {
  try {
    loading.value = true;
    await router.post(
      route("apps.deactivate", installation.id),
      {},
      {
        preserveScroll: true,
        onSuccess: () => {
...
const handleUninstall = async (installation) => {
  if (
    !confirm(
      t("apps.messages.confirm_uninstall", { name: installation.app.name }),
    )
  ) {
    return;
  }
  ...
  try {
    ...
    await router.delete(route("apps.uninstall", installation.app.id), {
      preserveScroll: true,
      onSuccess: () => {
        const index = props.installedApps.findIndex(
          (ia) => ia.id === installation.id,
        );
        if (index !== -1) {
...
const bulkUninstall = async () => {
  ...
  if (
    !confirm(
      t("apps.messages.confirm_bulk_uninstall", {
        count: installationsToUninstall.length,
      }),
    )
  ) {
    return;
