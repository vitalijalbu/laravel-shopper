  ...
  <div>
    ...
    <form @submit.prevent="submit" class="space-y-8">
      ...
      <div class="flex justify-end space-x-3">
        ...
        >
          {{ form.processing ? 'Creating...' : 'Create Product Type' }}
        </button>
      </div>
    </form>
  </div>
...
const generateSlug = debounce(() => {
  if (form.name && !props.productType) {
    form.slug = form.name
      .toLowerCase()
      .replace(/[^a-z0-9\s-]/g, '')
      .replace(/\s+/g, '-')
...
const submit = () => {
  if (props.productType) {
    ...
    form.post(route('cp.product-types.store'))
  }
}
...
const submit = async () => {
  if (props.productType) {
    form.put(`/cp/product-types/${props.productType.id}`)
  } else {
    form.post('/cp/product-types')
  }
}
