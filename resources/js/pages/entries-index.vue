<template>
  <page 
    ...
    :subtitle="collection.description || `Manage ${collection.title.toLowerCase()}`"
    ...
    <template #actions>
      <div class="entry-actions">
        ...
        <div v-if="selectedEntries.length > 0" class="bulk-actions">
          ...
          <button class="btn btn-outline btn-sm" @click="bulkPublish" v-if="canPublish">
            Publish
          </button>
          
          <button class="btn btn-outline btn-sm" @click="bulkUnpublish" v-if="canPublish">
            Unpublish  
          </button>
          
          <button class="btn btn-danger btn-sm" @click="bulkDelete" v-if="canDelete">
            Delete
          </button>
        </div>
        ...
        <div v-else class="regular-actions">
          <button class="btn btn-outline" @click="exportEntries" v-if="canExport">
    ...
    <div class="entries-container">
      <div class="entries-toolbar">
        ...
        <div class="filters-section">
          ...
          <div v-if="isMultisite" class="filter-group">
            ...
            <select v-model="filters.site" class="filter-select" @change="applyFilters">
              <option value="">All Sites</option>
              <option v-for="site in collection.sites" :key="site" :value="site">
                {{ getSiteName(site) }}
              </option>
            </select>
          </div>
          ...
          <div v-if="isProductCollection" class="filter-group">
            ...
            <select v-model="filters.stock_status" class="filter-select" @change="applyFilters">
          ...
          <div v-if="isOrderCollection" class="filter-group">
            ...
            <select v-model="filters.order_status" class="filter-select" @change="applyFilters">
      ...
      <div v-if="!loading && entries.length === 0" class="empty-entries">
        <div class="empty-icon">
          <icon :name="collection.icon || 'document'" class="empty-state-icon" />
        </div>
        <h3 class="empty-title">No {{ collection.title.toLowerCase() }} yet</h3>
        <p class="empty-description">
          Create your first {{ collection.singular?.toLowerCase() || 'entry' }} to get started.
        </p>
        <button 
...
const loadEntries = async () => {
  try {
    ...
    if (searchQuery.value) {
      filteredEntries = shopperStore.searchEntries(searchQuery.value, collectionHandle)
    }
    
    if (hasActiveFilters.value) {
      filteredEntries = shopperStore.filterEntries(filters.value, collectionHandle)
    }
    
    // Sort
    filteredEntries = shopperStore.sortEntries(filteredEntries, sortBy.value, sortDirection.value)
...
const bulkPublish = async () => {
  ...
  shopperStore.addToast(`${selectedEntries.value.length} entries published`, 'success')
...
const bulkUnpublish = async () => {
  ...
  shopperStore.addToast(`${selectedEntries.value.length} entries unpublished`, 'success')
  selectedEntries.value = []
  loadEntries()
}
...
const bulkDelete = async () => {
  ...
  if (confirm(`Delete ${selectedEntries.value.length} entries?`)) {
    shopperStore.addToast(`${selectedEntries.value.length} entries deleted`, 'success')
    selectedEntries.value = []
    loadEntries()
  }
}
