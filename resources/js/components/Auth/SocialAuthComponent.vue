<template>
    <div class="social-auth">
                ...
                >
                    <div class="social-provider-content">
                        ...
                        <span class="social-provider-text">
                            {{ $t(authMode === 'register' ? 'social.actions.register_with' : 'social.actions.login_with', { provider: provider.name }) }}
                        </span>
        ...
        <div v-if="showDivider && availableProviders.length > 0" class="social-divider">
        ...
        <div v-if="user && showConnectedAccounts" class="connected-accounts">
            ...
            <div v-if="connectedProviders.length > 0" class="connected-providers">
                ...
                >
                    <div class="connected-provider-info">
                        ...
                        <div class="connected-provider-details">
                            ...
                            <div class="connected-provider-meta">
                                {{ $t('social.labels.linked_on', { date: formatDate(account.linked_at) }) }}
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        {{ unlinkingProvider === account.provider ? $t('social.status.unlinking') : $t('social.buttons.unlink') }}
                    </button>
                </div>
            </div>
        ...
        <div v-if="success" class="social-success">
            <div class="social-success-content">
                <svg class="social-success-icon" fill="currentColor" viewBox="0 0 20 20">
...
const loadProviders = async () => {
    try {
        const response = await fetch(route(props.apiMode ? 'api.auth.social.providers' : 'auth.social.providers'))
        const data = await response.json()
        
        if (data.success) {
            availableProviders.value = Object.entries(data.data.providers).map(([key, provider]) => ({
                key,
                name: provider.name,
...
const handleSocialAuth = async (provider) => {
    ...
    try {
        ...
        handleError(t('social.messages.errors.authentication_failed', { provider }))
    } finally {
        loading.value = false
...
const handleApiAuth = async (provider) => {
    // Get redirect URL
    const redirectResponse = await fetch(route('api.auth.social.redirect', provider), {
        method: 'POST',
        headers: {
