<template>
  <div
    class="installed-app-card"
    :class="{ 'system-app': installation.app.system }"
  >
    <!-- Selection Checkbox -->
    ...
    <div class="card-content">
      ...
      <div class="installation-info">
        <div class="info-item">
          <span class="info-label"
            >{{ t("apps.installed.installed_on") }}:</span
          >
        ...
        <div v-if="installation.last_used_at" class="info-item">
          ...
          <span class="info-value">{{
            formatDate(installation.last_used_at)
          }}</span>
        </div>
      ...
      <div v-if="installation.analytics" class="quick-stats">
        <div class="stat-item">
          <div class="stat-value">
            {{ formatNumber(installation.analytics.api_calls || 0) }}
          </div>
          <div class="stat-label">{{ t("apps.analytics.api_calls") }}</div>
        </div>
        <div class="stat-item">
          <div class="stat-value">
            {{ (installation.analytics.uptime || 0).toFixed(1) }}%
          </div>
          <div class="stat-label">{{ t("apps.analytics.uptime") }}</div>
        </div>
        <div class="stat-item">
          <div class="stat-value">
            {{ (installation.analytics.success_rate || 0).toFixed(1) }}%
          </div>
      ...
      <div v-if="hasWarnings" class="warnings-section">
        ...
        <div
          v-if="subscriptionExpiring"
          class="warning-item subscription-warning"
        >
          <ClockIcon class="warning-icon" />
          <span>{{
            t("apps.subscription.expires_on", {
              date: formatDate(installation.subscription_expires_at),
            })
          }}</span>
        ...
        <div v-if="hasErrors" class="warning-item error-warning">
          ...
          <span
            >{{ t("apps.analytics.errors") }}:
            {{ installation.analytics.error_count }}</span
          >
      ...
      <div class="secondary-actions">
        ...
        <div class="dropdown" ref="dropdownRef">
          ...
          <div v-if="showDropdown" class="dropdown-menu">
            ...
            <button @click="exportSettings" class="dropdown-item">
              <DocumentArrowDownIcon class="dropdown-icon" />
              {{ t("apps.actions.export_settings") }}
            </button>
            <button @click="resetSettings" class="dropdown-item">
...
const hasSubscription = computed(() => {
  return (
    props.installation.subscription_status && props.installation.app.price > 0
  );
});
...
const hasUpdate = computed(() => {
  return (
    props.installation.available_version &&
