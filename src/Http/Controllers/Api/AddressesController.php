...
use Cartino\DTO\AddressDTO;
use Cartino\DTO\BulkOperationDTO;
use Cartino\Http\Requests\Api\BulkActionRequest;
use Cartino\Http\Requests\Api\CreateManyAddressesRequest;
use Cartino\Http\Requests\Api\DestroyManyAddressesRequest;
use Cartino\Http\Requests\Api\StoreAddressRequest;
use Cartino\Http\Requests\Api\UpdateAddressRequest;
use Cartino\Http\Requests\Api\UpdateManyAddressesRequest;
use Cartino\Http\Resources\AddressResource;
use Cartino\Models\Address;
use Cartino\Repositories\AddressRepository;
...
class AddressesController extends ApiController
{
    ...
    public function show(int|string $handle): JsonResponse
    {
        ...
        return $this->successResponse(new AddressResource($data));
    }
    ...
    public function store(StoreAddressRequest $request): JsonResponse
    {
        try {
            ...
            return $this->created(new AddressResource($brand), 'Address creato con successo');
        } catch (\Exception $e) {
     ...
     */
    public function createMany(CreateManyAddressesRequest $request): JsonResponse
    {
        try {
            $brandDTOs = collect($request->getAddressesData())
                ->map(fn (array $data) => AddressDTO::fromArray($data))
                ->toArray();

            $brandsData = collect($brandDTOs)->map(fn (AddressDTO $dto) => $dto->toCreateArray())->toArray();

            $brands = $this->repository->createMany($brandsData);

            return $this->created(AddressResource::collection($brands), count($brands).' brand creati con successo');
        } catch (\Exception $e) {
     ...
     */
    public function update(UpdateAddressRequest $request, Address $brand): JsonResponse
    {
        try {
            $updatedAddress = $this->repository->updateOne($brand->id, $request->validated());

            return $this->successResponse(new AddressResource($updatedAddress), 'Address aggiornato con successo');
        } catch (\Exception $e) {
     ...
     */
    public function updateMany(UpdateManyAddressesRequest $request): JsonResponse
    {
        try {
     ...
     */
    public function destroy(Address $brand): JsonResponse
    {
        try {
            ...
            return $this->successResponse(null, 'Address eliminato con successo');
        } catch (\Exception $e) {
     ...
     */
    public function destroyMany(DestroyManyAddressesRequest $request): JsonResponse
    {
        try {
            ...
            foreach ($bulkOperation->getIds() as $id) {
                if ($bulkOperation->isForceOperation() || $this->repository->canDelete($id)) {
                    ...
                    $errors[] = "Address ID {$id} non può essere eliminato: è associato a dei prodotti";
                }
            }
     ...
     * Toggle brand status
     */
    public function toggleStatus(Address $brand): JsonResponse
    {
        try {
            $updatedAddress = $this->repository->toggleStatus($brand->id);

            return $this->successResponse(new AddressResource($updatedAddress), 'Stato del brand aggiornato');
        } catch (\Exception $e) {
     ...
     * Get brand products
     */
    public function products(Address $brand): JsonResponse
    {
        $products = $this->repository->getAddressProducts($brand->id);
